...
为 AI 编程助手（Claude Code、Codex 等）构建的 Stata graphics 可信参考手册，尤其适合经济学实证研究场景。

A trusted Stata graphics reference manual for AI coding assistants (Claude Code, Codex, etc.), especially for empirical economics research.
...
当你让 AI 编程助手写 Stata 画图代码时，理想状态不是让它凭训练记忆临场拼代码，而是先查阅一组结构化、经过整理的 Stata graphics 参考资料，再根据具体项目调整。

这个项目的重点不是替 AI 兜底，而是给 AI 一个可靠的信息源。它把常用、易混、需要反复查阅的 Stata 图形语法、选项和模板整理成 Markdown 文件，方便 AI 助手在生成代码前读取。

**没有 skill 时：** AI 主要依赖训练记忆和临场推断。

**有 skill 后：** AI 先查阅参考手册，再结合用户数据、模型设定和项目风格写代码。

### 覆盖范围

**估计结果可视化**（最高优先级，也是最需要结构化参考的场景）：
...
相比之下，没有 skill 时 CC 可能会直接输出代码，而不会先查阅结构化参考文件。
...
When you ask an AI coding assistant to write Stata graphics code, the ideal workflow is not for the assistant to assemble code from model memory alone. It should first consult a structured, curated set of Stata graphics references, then adapt the code to the user's data, model, and project style.

This project is not mainly a safety net for AI mistakes. It gives the assistant a reliable information source: common, subtle, and frequently consulted Stata graphics syntax, options, and templates are organized into Markdown files that the assistant can read before writing code.

**Without this skill:** AI relies mainly on model memory and ad hoc inference.

**With this skill:** AI consults a trusted reference manual first, then writes code for the user's specific empirical task.

### What's Covered

**Estimation result visualization** (highest priority — the area that most needs structured reference material):
...
Without the skill, CC may write code immediately instead of consulting a structured reference source first.
